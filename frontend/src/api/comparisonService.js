...
/**  
 * Get summary of budget vs actual  
 */  
export const getBudgetVsActualSummary = async (month) => {  
  try {  
    ...
    const underBudgetCount = comparison.filter(  
      (c) => c.status === 'under'  
    ).length;  
    const onTrackCount = comparison.filter((c) => c.status === 'on-track')  
      .length;  
...
/**  
 * Get recommendations based on budget vs actual  
 */  
export const getSpendingRecommendations = async (month) => {  
  try {  
    const comparison = await getBudgetVsActual(month);  
