      ...
      <div className="mx-auto max-w-6xl space-y-5 px-4 py-5 md:px-8">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="rounded-2xl bg-[#5BAEE8] px-5 py-3 text-sm font-semibold text-white disabled:bg-[#D8E9F6] disabled:text-[#6F8A9A]"
          >
            + Income
          </button>
          <button
          ...
          <div className="ml-auto flex items-center gap-3 rounded-2xl border border-black/10 bg-white px-4 py-3 shadow-sm">
            <button onClick={() => setMonth((current) => shiftMonth(current, -1))} className="text-xs font-semibold text-[#7B6E67]">
              Prev
            </button>
        ...
        {summary ? (
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm">
              <p className="text-xs text-[#7B6E67]">Total income</p>
              <p className="mt-1 text-2xl font-bold text-[#1A2B38]">{formatCurrency(summary.totalIncome)}</p>
            </div>
            <div className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm">
              <p className="text-xs text-[#7B6E67]">Total expenses</p>
              <p className="mt-1 text-2xl font-bold text-[#FF6B58]">{formatCurrency(summary.totalExpenses)}</p>
            </div>
            <div className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm">
              <p className="text-xs text-[#7B6E67]">Savings</p>
              <p className={`mt-1 text-2xl font-bold ${summary.savings >= 0 ? "text-[#29B9AA]" : "text-[#FF6B58]"}`}>{formatCurrency(summary.savings)}</p>
            </div>
          </div>
        ) : null}
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <div className="rounded-[32px] border border-black/10 bg-white p-5 shadow-sm">
            <div className="mb-3 flex items-center justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#7B6E67]">Income by source</p>
          ...
          <div className="rounded-[32px] border border-black/10 bg-white p-5 shadow-sm">
            <div className="mb-3">
              <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#7B6E67]">Income sources</p>
        ...
        <div className="rounded-[32px] border border-black/10 bg-white p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#7B6E67]">Recent transactions</p>
              ...
              transactions.slice(0, 10).map((transaction) => (
                <div key={transaction.id} className="flex flex-wrap items-center gap-4 rounded-2xl bg-[#FEF9F4] px-4 py-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => handleDeleteTransaction(transaction.id)} className="rounded-full bg-red-50 px-3 py-2 text-xs font-semibold text-[#FF6B58]">
                      Delete
                    </button>
                  </div>
                </div>
